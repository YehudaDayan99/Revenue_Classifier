#!/usr/bin/env python3
"""
Revenue Extraction Validator Dashboard v0.3

Changes in v0.3:
- Extracted Data: proper table with row-highlight selection (no buttons)
- Description panel moved below Extracted Data + 10-K Source (full-width)
- 10-K viewer expanded into freed column space
- Cleaner CSS for selected-row highlighting
...
def inject_css():
    ...
    /* Selected-row highlight in the extracted-data table */
    .extracted-table { width: 100%; border-collapse: collapse; font-size: 0.9rem; }
    .extracted-table th {
        text-align: left; padding: 6px 10px;
        border-bottom: 2px solid #555; color: #aaa; font-weight: 600;
    }
    .extracted-table td {
        padding: 6px 10px; border-bottom: 1px solid #333; cursor: pointer;
    }
    .extracted-table tr:hover td { background: #2a2a3a; }
    .extracted-table tr.selected td { background: #1a3a5c; border-left: 3px solid #4a9eff; }
    .extracted-table .rev-val { text-align: right; font-variant-numeric: tabular-nums; }
    .extracted-table .total-row td {
        border-top: 2px solid #555; font-weight: 700; padding-top: 8px;
    }

    /* Description panel styling */
    .desc-panel {
        background: #181828; border: 1px solid #333; border-radius: 6px;
        padding: 16px 20px; margin-top: 4px;
    }
    .desc-panel h4 { margin: 0 0 8px 0; color: #4a9eff; }
    .desc-panel .desc-text { color: #ccc; line-height: 1.5; }
    .desc-panel .desc-meta { color: #888; font-size: 0.82rem; margin-top: 8px; }
    </style>
...
def render_metrics_card(metrics: Dict[str, Any]):
    ...
    with col2:
        ...
        st.metric("Extracted Total", f"${extracted:,.0f}M" if extracted else "—")
    with col3:
        expected = metrics.get("expected_total")
        if expected:
            val = expected / 1e6 if expected > 1e9 else expected
            st.metric("Expected Total", f"${val:,.0f}M")
        else:
    ...
    with col4:
        ...
        if delta is not None:
            st.metric("Delta", f"{delta:.2f}%",
                       delta_color="normal" if abs(delta) < 2 else "inverse")
        else:
            st.metric("Delta", "—")


def render_extracted_table(lines: list) -> Optional[str]:
    """Render extracted line items as an HTML table with click-to-select via
    st.radio hidden behind clean row rendering.

    Returns the selected line name (or None).
    """
    if not lines:
    ...
    # --- Use st.dataframe selection (Streamlit ≥1.35) with fallback ---
    display_df = pd.DataFrame({
        "Revenue Line": df.get("line", df.get("segment", "")),
        "Revenue ($M)": df["revenue"].apply(
            lambda x: f"${x:,.0f}" if pd.notna(x) else "—"
        ),
        "Segment": df.get("segment", "").apply(lambda x: (x or "")[:35]),
    })

    try:
        # Modern Streamlit: native row selection
        event = st.dataframe(
            display_df,
        ...
        # Fallback for older Streamlit: radio selector
        line_labels = [
            f"{l.get('line', 'Unknown')}  —  ${l['revenue']:,.0f}M"
            if l.get("revenue") else l.get("line", "Unknown")
            for l in lines
        ]
        choice = st.radio(
            "Select line item",
...
def render_filing_viewer(ticker: str, ticker_data: Dict[str, Any]):
    """Render 10-K source — expanded to fill right column."""
    ...
    with tab2:
        if filing_html:
            ...
            container = f"""
            <div style="height:700px; overflow-y:auto; border:1px solid #444;
            ...
            c1, c2 = st.columns(2)
            with c1:
                st.link_button("🔗 Open in SEC EDGAR", edgar_url, use_container_width=True)
            with c2:
                if st.button("📥 Cache Filing Locally", use_container_width=True):
                    st.info(f"Download 10-K HTML from EDGAR → save to data/filings/{ticker}_10K.html → refresh")


def render_description_panel(lines: list, selected_line: Optional[str]):
    """Full-width description panel below the two-column layout."""
    if not selected_line:
        st.markdown(
            '<div class="desc-panel"><span style="color:#888;">👆 Click a row in Extracted Data to see its description</span></div>',
            unsafe_allow_html=True,
        )
        return
    ...
    if not line_data:
        st.warning(f"No data for: {selected_line}")
        return
    ...
    meta_parts = [f"Segment: {segment}" if segment else None,
                  f"Revenue: {rev_str}",
...
def render_review_controls(ticker: str, review_state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    with col2:
        if st.button("❌ Reject", key=f"reject_{ticker}"):
            ...
            ticker_state["reviewed_at"] = datetime.now().isoformat()
    with col3:
        icon = {"approved": "✅ Approved", "rejected": "❌ Rejected",
                "pending": "⏳ Pending Review"}.get(ticker_state.get("status", "pending"), "")
        st.markdown(f"**Status:** {icon}")
        if ticker_state.get("reviewed_at"):
            st.caption(f"Reviewed: {ticker_state['reviewed_at'][:16]}")

    ticker_state["notes"] = st.text_area(
        "Notes", value=ticker_state.get("notes", ""),
        key=f"notes_{ticker}", placeholder="Review notes...", height=68,
    )
    return ticker_state
...
def render_ticker_detail(ticker: str, ticker_data: Dict[str, Any], review_state: Dict[str, Any]) -> Dict[str, Any]:
    """Main detail view — restructured layout for v0.3."""
    st.header(f"📄 {ticker}")
    ...
    # ── Two-column top: Extracted Data (narrow) | 10-K Source (wide) ──
...
def main():
    st.set_page_config(page_title="Revenue Extraction Validator", page_icon="📊", layout="wide")
    inject_css()
    ...
    view_mode = st.sidebar.radio("View", ["Summary", "Detail"], horizontal=True)
    ...
    if view_mode == "Detail":
        ...
        # Clear line selection on ticker change
        ...
        updated = render_ticker_detail(selected_ticker, data["tickers"][selected_ticker], review_state)
