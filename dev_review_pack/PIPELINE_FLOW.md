...
| `choose_item_col()` | `react_agents.py` | **P1**: Deterministic label column selection |
...
1. **Phase 1: NVDA item_col Fix** (`react_agents.py`):
   ...
   - Label column sanity gate rejects columns with >50% numeric values
   - **Result**: NVDA labels now correct (Gaming, ProViz, Automotive working)

2. **Phase 2: AMZN Footnote Extraction** (`react_agents.py`):
   - `extract_footnotes_from_dom_context()` uses DOM-based extraction with normalized text
   - Handles iXBRL split tags (e.g., `<ix:nonFraction>` around numbers)
   - **Result**: All 7 AMZN revenue lines have descriptions from footnotes (1)-(6)

3. **Phase 3: AAPL Services Subheading** (`react_agents.py`):
   - `_extract_heading_based_definition()` enhanced for subheading aggregation
   - Wraps HTML in `<div>` to handle edge cases (closing tags at start)
   - **Result**: Services description covers Advertising, AppleCare, Cloud, Digital Content, Subscriptions

4. **Phase 4: Provenance Artifact** (`pipeline.py`, `react_agents.py`):
   - `csv1_desc_provenance.json` tracks source_section, evidence_snippet, footnote_id
...
7. **Phase 7: Dev Review Correctness Fixes** (Jan 2026):
   - **P0.1**: Validation now fails immediately when `table_total` exists and mismatches (no fall-through to permissive fallback)
   - **P0.2**: META double-counting fixed by adding segment names to `META_SUBTOTAL_ITEMS` and smart deduplication in extraction
   - **P0.3**: Adjustment rows (e.g., GOOGL "Hedging gains/losses") now emitted to CSV1 for 100% reconciliation
   - **P1.4**: Description hygiene - ToC/page artifact stripping, expanded denylist patterns
   - **P1.5**: NVDA Automotive now mapped to "Compute & Networking" per Item 1 segment definition
   - **P2.6**: Added 7 regression tests in `tests/test_dev_review_regressions.py`
   - **Result**: All 6 tickers validate to 0.00% delta

8. **Phase 8: Offerings Pattern Extraction** (Jan 2026):
   - Added "offerings include" pattern search (Phase 5a) BEFORE note2-paragraph extraction
   - Pattern: `{item} offerings include [description].`
   - Produces richer, item-specific definitions vs. generic segment enumerations
   - **Result**: NVDA Compute/Networking now have dedicated product definitions instead of segment fragments

9. **Phase 9: Table Rejection Improvements** (Jan 2026):
   - **P0**: Added context-negative patterns to `table_kind.py`:
     - Stock performance tables (`$100 invested`, `S&P 500 Index`, `cumulative total return`)
     - Earnings tables (`net income`, `earnings before tax`, `segment earnings`)
     - Volume metrics (`gross dollar volume`, `payment volume`, `processed transactions`)
   - **P0.5**: Added sanity gates in `pipeline.py`:
     - Reject all-zero value tables (stock charts extracting as $0)
     - Reject percent tables (when values ≤ 5 and "%" in headers)
   - **Result**: Stock charts, earnings tables, volume tables now rejected before LLM selection

10. **Phase 10: Post-LLM Layout Validators** (Jan 2026):
    - **P0.1 Sum-as-Total**: When no explicit total row exists, use sum of extracted items for validation
    - **P0.2 Year Column Scoring**: Detect and avoid % change columns, prefer adjacent $ columns
    - **P0.3 Segment_col Validation**: Nullify LLM's `segment_col` if match_rate < 30% vs expected segments
    - **P0.4 Auto-Scaling**: Detect 1e3/1e6 magnitude errors by comparing to SEC external total
    - **P0.5+ Row Label QA**: Reject date-based labels and employee/headcount tables
    - **P0.5+ Cost/Margin Filter**: Exclude cost/expense/margin rows from CSV output

11. **Phase 11: External Validation Hard Gate** (Feb 2026):
    - **P0.1 External Total HARD GATE**: After scaling, extracted total MUST match SEC external total within 10%
      - Uses multiple XBRL tag fallbacks: `Revenues`, `SalesRevenueNet`, `RevenueFromContractWithCustomer*`
      - Rejects tables that pass internal consistency but are economically wrong
    - **P0.2 Header-Based Column Veto**: Reject columns with headers matching `%/change/variance/increase/decrease`
      - Fixes: AVGO (was selecting % Change column instead of $ value)
      - Scans all header rows, not just cell values
    - **P0.3 Pre-Validation Filtering**: Apply cost/margin filter BEFORE validation (align validation with output)
      - Previously: validated one set, output different set → hidden mismatches
      - Now: validate exactly what we emit
    - **P0.5 Total Row Deduplication**: Remove "Net revenue"/"Total revenue" rows when ≥3 other items exist
      - Fixes: V, MA (were including total alongside components)
    - **Result**: 15/20 tickers pass with honest external validation (75%)

### Remaining Gaps

| Issue | Affected Tickers | Root Cause | Status |
|-------|------------------|------------|--------|
| **Missing revenue items** | TSLA | 17.9% mismatch - some automotive items not extracted | Needs investigation |
| **Mixed dimensions** | MA | Geography + service lines mixed → double counting | Architecture limitation |
| **Bank structure** | JPM | Net interest income doesn't fit product/segment model | Out of scope for PoC |
| **Fiscal year mismatch** | WMT | 32% mismatch - may be FY timing | Needs investigation |
| **Conglomerate** | BRK-B | Many diverse business lines, no single disaggregation | Architecture limitation |
