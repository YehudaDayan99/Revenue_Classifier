...
def _clear_caches() -> None:
    ...
    # Clear soup cache from table_candidates (imported below to avoid circular import)
    from revseg.table_candidates import clear_soup_cache
    clear_soup_cache()
        ...
        try:
            ...
            for attempt_id in candidate_table_ids:
                ...
                try:
                    ...
                    # P0.3: Validate segment_col - if values don't match expected segments, nullify it
                    seg_col = layout.get("segment_col")
                    if seg_col is not None:
                        seg_vals = set()
                        for row in grid:
                            if seg_col < len(row):
                                val = str(row[seg_col]).strip().lower()
                                if val and len(val) > 1:
                                    seg_vals.add(val)
                        # Check match rate against expected segments
                        expected_lower = {s.lower() for s in all_segments}
                        matches = sum(1 for v in seg_vals if any(e in v or v in e for e in expected_lower))
                        match_rate = matches / max(len(seg_vals), 1)
                        if match_rate < 0.3 and len(seg_vals) > 2:
                            print(f"[{ticker}] segment_col nullified: match_rate={match_rate:.2f}", flush=True)
                            # Move segment_col to item_col if item_col not set
                            if layout.get("item_col") is None:
                                layout["item_col"] = seg_col
                            layout["segment_col"] = None
                            _trace_append(t_art, {"stage": "segment_col_nullified", "match_rate": match_rate})
                    
                    # P0.2: Validate year columns - prefer $ columns over % columns
                    # Enhanced with header-based veto for change/variance columns (fixes AVGO)
                    year_cols = layout.get("year_cols", {})
                    _CHANGE_COL_PATTERN = re.compile(r'\b(%|percent|change|variance|increase|decrease)\b', re.IGNORECASE)
                    header_rows = layout.get("header_rows", [0])
                    if year_cols and grid:
                        for year_key, col_idx in list(year_cols.items()):
                            if col_idx is None or col_idx >= len(grid[0]):
                                continue
                            
                            # P0.2 FIX: Check column header specifically for change/variance language
                            col_header_blob = " ".join(
                                str(grid[r][col_idx]) if r < len(grid) and col_idx < len(grid[r]) else ""
                                for r in header_rows if r < len(grid)
                            )
                            is_change_col = bool(_CHANGE_COL_PATTERN.search(col_header_blob))
                            
                            # Score this column for "money-likeness"
                            col_vals = [row[col_idx] if col_idx < len(row) else "" for row in grid]
                            pct_count = sum(1 for v in col_vals if '%' in str(v))
                            
                            # If this column looks like percentages or change column, try adjacent columns
                            if is_change_col or pct_count > len(col_vals) * 0.3:
                                reason = "change_column" if is_change_col else "percent_values"
                                # Try column to the left (often $ is before % or $ Change)
                                alt_col = col_idx - 1
                                while alt_col >= 0:
                                    # Check if alt_col is also a change column
                                    alt_header = " ".join(
                                        str(grid[r][alt_col]) if r < len(grid) and alt_col < len(grid[r]) else ""
                                        for r in header_rows if r < len(grid)
                                    )
                                    alt_is_change = bool(_CHANGE_COL_PATTERN.search(alt_header))
                                    alt_vals = [row[alt_col] if alt_col < len(row) else "" for row in grid]
                                    alt_pct = sum(1 for v in alt_vals if '%' in str(v))
                                    
                                    if not alt_is_change and alt_pct < pct_count:
                                        print(f"[{ticker}] year_col override: col {col_idx} -> {alt_col} ({reason})", flush=True)
                                        year_cols[year_key] = alt_col
                                        _trace_append(t_art, {"stage": "year_col_override", "year": year_key, "from": col_idx, "to": alt_col, "reason": reason})
                                        break
                                    alt_col -= 1
                    ...
                    # P0.5+ Row-label QA: Reject date-based or employee/headcount tables
                    date_pattern = re.compile(r'\b(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\.?\s+\d|^\d{1,2}/\d{1,2}/\d{2,4}$', re.IGNORECASE)
                    employee_pattern = re.compile(r'\b(?:employees?|fte|headcount|full[- ]time)\b', re.IGNORECASE)
                    date_labels = sum(1 for lbl in extracted_labels if date_pattern.search(lbl))
                    employee_labels = sum(1 for lbl in extracted_labels if employee_pattern.search(lbl))
                    if date_labels > len(extracted_labels) * 0.3:
                        print(f"[{ticker}] REJECT table {attempt_id}: date-based labels detected", flush=True)
                        _trace_append(t_art, {"stage": "label_qa_fail", "table_id": attempt_id, "reason": "date_labels", "sample": extracted_labels[:3]})
                        continue
                    if employee_labels > 0 and employee_labels >= len(extracted_labels) * 0.2:
                        print(f"[{ticker}] REJECT table {attempt_id}: employee/headcount table detected", flush=True)
                        _trace_append(t_art, {"stage": "label_qa_fail", "table_id": attempt_id, "reason": "employee_labels", "sample": extracted_labels[:3]})
                        continue
                    
                    # P0.3 FIX: Apply cost/margin filtering BEFORE validation (align validation with output)
                    _COST_MARGIN_PATTERN_PRE = re.compile(
                        r'\b(cost\s+of|gross\s+margin|operating\s+(income|expense|loss)|'
                        r'research\s+and\s+development|selling.*general|amortization|'
                        r'restructuring|gross\s+profit|total\s+cost|percent\s+increase|'
                        r'total\s+operating\s+expenses?|total\s+expenses?|'
                        r'income\s+from\s+operations|net\s+income|depreciation)\b',
                        re.IGNORECASE
                    )
                    pre_filter_count = len(result.rows)
                    result.rows = [r for r in result.rows if not _COST_MARGIN_PATTERN_PRE.search(r.item)]
                    if len(result.rows) < pre_filter_count:
                        # Recompute segment_revenues from filtered rows
                        result.segment_revenues = {
                            r.item: r.value for r in result.rows 
                            if r.row_type in ("item", "segment", "unknown") and r.value > 0
                        }
                        result.adjustment_revenues = {
                            r.item: r.value for r in result.rows 
                            if r.row_type == "adjustment"
                        }
                        print(f"[{ticker}] pre-validation filter: removed {pre_filter_count - len(result.rows)} cost/margin rows", flush=True)
                    
                    # P0.5 FIX: Remove total rows when ≥3 revenue items exist (fix V/MA duplication)
                    _TOTAL_REVENUE_PATTERN = re.compile(
                        r'^(total\s+)?(net\s+)?(revenue|sales)s?$|^net\s+revenue$',
                        re.IGNORECASE
                    )
                    non_total_items = [r for r in result.rows if r.row_type != "total" and not _TOTAL_REVENUE_PATTERN.match(r.item.strip())]
                    if len(non_total_items) >= 3:
                        # We have enough items, remove "Net revenue" / "Total revenue" rows
                        total_rows_removed = [r.item for r in result.rows if _TOTAL_REVENUE_PATTERN.match(r.item.strip())]
                        if total_rows_removed:
                            result.rows = [r for r in result.rows if not _TOTAL_REVENUE_PATTERN.match(r.item.strip())]
                            # Recompute segment_revenues
                            result.segment_revenues = {
                                r.item: r.value for r in result.rows 
                                if r.row_type in ("item", "segment", "unknown") and r.value > 0
                            }
                            print(f"[{ticker}] dedup: removed total rows {total_rows_removed}", flush=True)
                    
                    # P0.5 Sanity gate: Reject all-zero tables (stock charts, wrong tables)
                    vals = [abs(v) for v in result.segment_revenues.values() if v is not None]
                    if not vals or max(vals) == 0:
                        print(f"[{ticker}] REJECT table {attempt_id}: all values are zero", flush=True)
                        _trace_append(t_art, {"stage": "sanity_reject", "table_id": attempt_id, "reason": "all_zero_values"})
                        continue
                    
                    # P0.5 Sanity gate: Reject percent tables (values sum to ~100 or all small)
                    text_hint = " ".join([
                        table_caption or "",
                        table_heading or "",
                        str(layout.get("notes", "") or ""),
                        " ".join(grid[0] if grid else [])
                    ]).lower()
                    if ("%" in text_hint or "percent" in text_hint) and max(vals) <= 5:
                        print(f"[{ticker}] REJECT table {attempt_id}: appears to be percent table", flush=True)
                        _trace_append(t_art, {"stage": "sanity_reject", "table_id": attempt_id, "reason": "percent_table_suspected"})
                        continue
                    
                    # Validate using self-consistent validation
                    external_total = fetch_companyfacts_total_revenue_usd(cik, result.year)
                    
                    # P0.1: Sum-as-total - if no explicit total row, use sum of extracted items
                    table_total_for_validation = result.table_total
                    if (table_total_for_validation is None or table_total_for_validation == 0) and len(result.segment_revenues) >= 2:
                        table_total_for_validation = sum(result.segment_revenues.values())
                        print(f"[{ticker}] sum-as-total: using extracted sum {table_total_for_validation:,} as table_total", flush=True)
                        _trace_append(t_art, {"stage": "sum_as_total", "computed_total": table_total_for_validation})
                    
                    # P0.4: Units auto-scaling - detect if values are off by 1e3 or 1e6
                    scale_factor = 1
                    if external_total and table_total_for_validation and table_total_for_validation > 0:
                        ratio = external_total / table_total_for_validation
                        if 5e5 < ratio < 2e6:
                            scale_factor = 1_000_000
                            print(f"[{ticker}] auto-scaling: ratio={ratio:.0f}, applying 1e6 scale", flush=True)
                        elif 5e2 < ratio < 2e3:
                            scale_factor = 1_000
                            print(f"[{ticker}] auto-scaling: ratio={ratio:.0f}, applying 1e3 scale", flush=True)
                        
                        if scale_factor > 1:
                            # Scale the extracted values
                            result.segment_revenues = {k: v * scale_factor for k, v in result.segment_revenues.items()}
                            result.adjustment_revenues = {k: v * scale_factor for k, v in result.adjustment_revenues.items()}
                            if result.table_total:
                                result.table_total *= scale_factor
                            table_total_for_validation *= scale_factor
                            # Also scale individual row values (for CSV output)
                            for row in result.rows:
                                row.value = int(row.value * scale_factor)
                            _trace_append(t_art, {"stage": "auto_scale", "scale_factor": scale_factor})
                    
                    # P0.1 FIX: External total HARD GATE (5% tolerance for PoC)
                    # After scaling, extracted total MUST match external total if available
                    if external_total and table_total_for_validation:
                        external_delta_pct = abs(table_total_for_validation - external_total) / external_total
                        if external_delta_pct > 0.10:  # 10% tolerance (allows for rounding/timing differences)
                            print(f"[{ticker}] REJECT table {attempt_id}: external_total mismatch {external_delta_pct:.1%} (extracted={table_total_for_validation:,}, SEC={external_total:,})", flush=True)
                            _trace_append(t_art, {"stage": "external_validation_fail", "table_id": attempt_id, 
                                                   "extracted": table_total_for_validation, "external": external_total, 
                                                   "delta_pct": f"{external_delta_pct:.2%}"})
                            continue
                    ...
                    attempt_validation = validate_extraction(
                        ...
                        table_total=table_total_for_validation,
            ...
            if extraction_result and extraction_result.rows:
                # Step 1: Filter rows (keep adjustments for reconciliation, exclude totals/subtotals)
                # P0.3 FIX: Include adjustment rows so sum(CSV1) = table_total
                eligible_rows = [
                    r for r in extraction_result.rows
                    if r.row_type != "total"  # Exclude only total rows
                    and not is_total_row(r.item)
                    and not is_subtotal_row(r.item, ticker)
                    and (r.value > 0 or r.row_type == "adjustment")  # Allow negative adjustments
                ]
                
                # P0.5+: Filter out cost/expense/margin rows (not revenue items)
                ...
                eligible_rows = [
                    ...
                    if not _COST_MARGIN_PATTERN.search(r.item)
                ]
                
                # Separate adjustment rows (they bypass dimension filtering)
                adjustment_rows = [r for r in eligible_rows if r.row_type == "adjustment"]
                non_adjustment_rows = [r for r in eligible_rows if r.row_type != "adjustment"]
                eligible_rows = non_adjustment_rows  # Continue with non-adjustments for dimension filtering
                ...
                # P0.3: Add adjustment rows back for reconciliation
                filtered_rows.extend(adjustment_rows)
                ...
                for r in sorted(filtered_rows, key=lambda x: (-abs(x.value), x.item)):
                    # Clean footnote markers from label
                    clean_label = _clean_revenue_line(r.item)
                    
                    # P0.3: Handle adjustment rows specially
                    if r.row_type == "adjustment":
                        revenue_group = "Adjustments / Reconciling items"
                        description = ""  # Adjustments don't need descriptions
                    else:
