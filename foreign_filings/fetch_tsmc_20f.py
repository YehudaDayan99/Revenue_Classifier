#!/usr/bin/env python3
"""Script to download TSMC's latest 20-F filing."""
...
def main():
    """Download TSMC's latest 20-F filing."""
    output_dir = Path(__file__).parent / "data" / "20f"
    cache_dir = Path(__file__).parent / "cache"
    
    print("Downloading TSMC (TSM) latest 20-F filing...\n")
    ...
    try:
        ...
        print(f"\n✓ Successfully downloaded TSMC 20-F filing to:")
        print(f"  {filing_path}")
        print(f"\nFiles saved:")
        ...
        print(f"\n✗ Failed to download TSMC 20-F filing:")
