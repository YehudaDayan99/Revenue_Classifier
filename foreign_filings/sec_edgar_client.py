"""SEC Edgar client for fetching filings of any form type (10-K, 20-F, 10-Q, etc.)"""
...
def select_latest_filing(
    ...
    *,
    ...
    """Select the most recent filing of a given form type (e.g., '10-K', '20-F').
    
    Args:
        ticker: Company ticker
        cik: CIK number
        submissions: Company submissions JSON from SEC
        form_type: Form type to search for (e.g., '10-K', '20-F', '10-Q')
        include_amendments: If True, also accept amended versions (e.g., '10-K/A', '20-F/A')
    """
    ...
    raise SecEdgarError(f"No {form_type} found for {ticker} (include_amendments={include_amendments})")
...
def download_latest_filing(
    ...
    *,
    ...
    """Download latest filing of a given form type and metadata. Returns output folder path.
    
    Args:
        ticker: Company ticker symbol
        out_dir: Output directory where filing will be saved
        form_type: Form type to download (e.g., '10-K', '20-F', '10-Q')
        include_amendments: If True, accept amended filings
        cache_dir: Optional cache directory for caching SEC data
        min_interval_s: Minimum interval (seconds) between SEC API calls
    
    Returns:
        Path to the folder containing the downloaded filing
    """
    ...
    filing = select_latest_filing(ticker, cik, subs, form_type, include_amendments=include_amendments)
